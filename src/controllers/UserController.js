...
export default class UserController extends BaseController {
  constructor (router, { User }) {
    ...
    this.router.post('/user/:userId/:provider', this.authorize('ADMIN'), this.postUser)
    this.router.delete('/user/:userId/:provider', this.authorize('ADMIN'), this.deleteUser)
  ...
  saveAuthenticate (provider, accessToken, refreshToken, profile, done) {
    ...
    this.userModel.findOne({ userId, provider })
      .then((resultData) => {
        ...
        if (resultData) {
          return this.userModel.findOneAndUpdate({ _id: resultData._id }, {
            $set: {
              userName,
              photoUrl,
              email,
